...
This guide covers the controls, hardware requirements, permissions, privacy,
and troubleshooting for the installed ASCII VJ Remix desktop app.

Start with the [install guide](../README.md#install-guide) and
[first-run steps](../README.md#first-run). See the
[documentation index](README.md) for development and reference guides.
...
- The production app checks GitHub Releases metadata for signed updater packages once
  ...
  [legacy updater recovery notes](#upgrading-from-096-or-097).
...
[UC-33e and mioXC guide](MIDI_UC33E.md).
...
- The published 1.0.4 FFmpeg/ffprobe sidecars were built with a macOS 26
  ...
  target in that release. The build correction is unreleased; older-OS runtime
...
See [Security](SECURITY.md) for the full local media, permission,
...
### Upgrading from 0.9.6 or 0.9.7

These versions shipped without the main-window capability used by the updater
availability gate, so their Update control can flash and then disappear. The
