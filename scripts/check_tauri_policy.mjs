...
const tauriAdapterPath = path.join(root, 'renderers', 'desktop', 'tauri-adapter.js');
const generatedPermissionsDir = path.join(root, 'src-tauri', 'permissions', 'autogenerated');
...
if (!mainCapability) {
  ...
  const requiredMainPermissions = [
    ...
    'allow-read-native-output-preview-frame',
  ...
  const tauriAdapter = await readFile(tauriAdapterPath, 'utf8');
  const invokedCommands = new Set(
    [...tauriAdapter.matchAll(/\binvokeTauri\(\s*['"]([a-z][a-z0-9_]*)['"]/g)]
      .map((match) => match[1])
  );
  for (const command of invokedCommands) {
    const permission = `allow-${command.replaceAll('_', '-')}`;
    if (!permissions.has(permission)) {
      issues.push(`main capability must allow frontend command ${command} with ${permission}`);
    }
    const permissionPath = path.join(generatedPermissionsDir, `${command}.toml`);
    let permissionSource = '';
    try {
      permissionSource = await readFile(permissionPath, 'utf8');
    } catch {
      issues.push(`frontend command ${command} is missing generated permission file ${command}.toml`);
      continue;
    }
    if (!permissionSource.includes(`commands.allow = ["${command}"]`)) {
      issues.push(`generated permission file ${command}.toml must allow ${command}`);
    }
  }
